...
public struct Validation<T>
{
    ...
    public async Task<TR> MatchAsync<TR>(Func<IEnumerable<Error>, TR> Invalid, Func<T, Task<TR>> Valid)
        => IsValid ? await Valid(this.Value) : Invalid(this.Errors);
...
public static class Validation
{
    ...
    public static async Task<Validation<RR>> MapAsync<R, RR>
        (this Validation<R> @this, Func<R, Task<RR>> f)
    {
        if (!@this.IsValid)
